...
static int IsTelNumStr(const char *s, const int size);
...
/*
 * 関数名			: int GetTelNumStr(char *s, const int size)
 * 機能				: 指定桁数（12桁）の数字のみの文字列を入力する
 * 入力パラメータ	: const int size	:桁数
 * 出力パラメータ	: char *s			:文字列
 * 戻り値			: int	:TRUE（指定桁数を入力）/FALSE（0,1のみ入力）
 * 特記事項			: ハイフンなしの電話番号を入力した場合には再入力を促す
 */
int GetTelNumStr(char* s, const int size)
{
	int retVal = TRUE;
	char buf[GETNUMSTR_BUF_SIZE];		/* 文字列入力バッファ */
	int len;							/* 入力した文字列の長さ */
	...
	while (TRUE) {
		...
		if (len == 0) {									/* エンターキーのみ */
			...
			retVal = FALSE;
			break;
		} else if(IsTelNumStr(buf, len) == FALSE) {
			printf("数字と半角ハイフンのみで入力してください。\n");		/*数字とハイフン以外が入力されたとき*/
			printf("入力＞　");
		} else if (len != size) {						/* 指定桁数以外 */
			printf("%d桁(ハイフン含む)で入力してください。\n", size);
			printf("入力＞　");
		} else {
			strncpy_s(s, size + 1, buf, _TRUNCATE);
			break;
...
/*
 * 関数名			: static int IsTelNumStr(const char *s, const int size)
 * 機能				: 電話番号に半角ハイフンが入っているかチェックする
 * 入力パラメータ	: const char *s		:文字列
 * 					: const int size	:桁数
 * 出力パラメータ	: なし
 * 戻り値			: int	:TRUE/FALSE
