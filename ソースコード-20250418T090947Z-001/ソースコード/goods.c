...
int WriteGoodsFile(void)
{
	...
	if(error != 0) {
		printf("\n商品情報マスタファイルに書き込みできません。処理を終了します。\n");
		exit(EXIT_FAILURE);
	}
...
int DisplayGoodsList(const int key, const int ud, const int menuNum)
{
	...
	strcpy_s(h[GOODS_DSP_LEND_NUM_COL], DISPLAYGOODSLIST_HEAD_COL_BUF_SIZE, GOODS_DSP_LEND_NUM_NAME);
	...
	sprintf_s(f[GOODS_DSP_LEND_NUM_COL], DISPLAYGOODSLIST_HEAD_FORMAT_COL_NUM, "%%-%ds ", GOODS_DSP_LEND_NUM_NUM);
	...
	while(target->next != NULL) {
		...
		strncpy_s(buf, GOODS_DSP_TITLE_NUM+1, target->title, _TRUNCATE);
		printf(f[GOODS_DSP_TITLE_COL], buf);
		strncpy_s(buf, GOODS_DSP_GENRE_NUM+1, target->genre, _TRUNCATE);
		...
		printf(f[GOODS_DSP_LEND_NUM_COL], target->lendNum);
		printf("\n");
		/* 表示ページを制御する */
		cnt++;
 ...
 * 機能				: 【D-1147】商品情報リストを指定キーで昇順/降順にソートする
 * 入力パラメータ	: const int key	:ソートキー
 * 					: const int ud	:昇順/降順（1:昇順/2:降順）
 * 出力パラメータ	: なし
 * 戻り値			: なし
		...
		while(TRUE) {
			...
			if(sTarget->next != NULL) {
				...
				case 3:				/* 3:発売日 */
					...
					break;
				case 4:				/* 4:貸出回数 */
					cmp = strncmp(newone->lendNum, sTarget->next->lendNum, GOODS_LEND_NUM_SIZE);
					break;
				default:			/* 例外 */
					cmp = 0;
					break;
